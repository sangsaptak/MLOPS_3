...
dataset['Age'] = dataset[['Age', 'Pclass']].apply(lwav , axis=1)
...
while num < 4 :
    ...
    accuracy = model.fit(X,y_cat, epochs=40)
    accuracy.history['accuracy'][-1:][0] 
    num += 1
...
if accuracy.history['accuracy'][-1:][0] <= 0.80 :
    os.system("curl --user admin:redhat http://192.168.56.101:8080/job/mlops6/build?token=mlops")
